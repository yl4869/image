...
// CF-BATCH 调度器：按输入顺序构建批次（连续相同尺寸合并），在每个批次内优先处理关键任务（crucial==1），不进行压缩
// 输入：CSV 文件（size,deadline,id,crucial,category），size 为 1-4（对应 64/128/256/512）
// 输出：output_cf_batch.json（当前目录），若存在未完成任务则生成 missed_tasks_cf_batch.json
...
int main(int argc, char *argv[]) {
    ...
    while (idx < task_count) {
        ...
        // 在批次内先处理关键任务，再处理非关键任务（保持原有相对顺序）
        int any_completed_in_batch = 0;
        int first_in_batch = 1;

        // 处理关键任务
        for (int j = batch_start; j <= batch_end; j++) {
            if (tasks[j].crucial != 1) continue;
    ...
    if (missed_count > 0) {
        ...
        if (mf) {
            ...
            fprintf(mf, "\n  ]\n}");
            fclose(mf);
            printf("[WARN] 有 %d 个任务错过截止期，详情已保存到 missed_tasks_cf_batch.json\n", missed_count);
        } else {
    ...
    free(tasks);
    printf("[OK] CF-BATCH 调度完成. 完成任务: %d, 错失: %d\n", completed_count, missed_count);
    return 0;
}
