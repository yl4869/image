...
# 配置参数
DEADLINE = 30.0  # 统一的截止时间
BASE_DIR = "images_cropped"  # 图片文件夹
OUTPUT_DIR = "task_files"  # 输出CSV文件的文件夹

# 不同大小对应的关键任务概率
CRUCIAL_PROB = {
    64: 0.2,   # 64大小：20%概率为关键任务
    128: 0.4,  # 128大小：40%概率为关键任务
    256: 0.6,  # 256大小：60%概率为关键任务
...
def generate_csv_for_folder(folder_path, output_csv_path):
    ...
    with open(output_csv_path, 'w', encoding='utf-8') as f:
        # 写入标题行
        f.write("size,deadline,id,crucial\n")
        
        # 写入数据行
        for task in tasks:
            f.write(f"{task['size']},{task['deadline']},{task['id']},{task['crucial']}\n")
