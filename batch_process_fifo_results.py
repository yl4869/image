...
def find_all_fifo_result_files(base_folder):
    """
    查找指定文件夹中所有子目录的 fifo_result.json 文件
    
    Args:
        base_folder: 基础文件夹路径
    
    Returns:
        包含 (fifo_result.json路径, fifo_time.json路径) 的元组列表
    """
    ...
    for result_dir in sorted(base_path.iterdir()):
        if result_dir.is_dir():
            fifo_result_path = result_dir / "fifo_result.json"
            if fifo_result_path.exists():
                fifo_time_path = result_dir / "fifo_time.json"
                file_pairs.append((str(fifo_result_path), str(fifo_time_path)))
...
def extract_cropped_folder_from_result_path(result_path):
    ...
    Args:
        result_path: 结果文件路径（如 result_153/fifo_result.json）
...
def batch_process_folder(base_folder, model_paths, num_classes=7, skip_existing=False):
    """
    批量处理指定文件夹中的所有 fifo_result.json 文件
    ...
    # 查找所有 fifo_result.json 文件
    file_pairs = find_all_fifo_result_files(base_folder)
    
    if not file_pairs:
        print(f"[!] 未找到任何 fifo_result.json 文件")
        return
    
    print(f"[OK] 找到 {len(file_pairs)} 个 fifo_result.json 文件\n")
...
def main():
    ...
    # 是否跳过已存在的 fifo_time.json 文件（True=跳过，False=覆盖）
    skip_existing = False
